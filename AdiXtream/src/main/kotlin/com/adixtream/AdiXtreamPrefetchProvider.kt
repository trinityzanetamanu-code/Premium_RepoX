...
class AdiXtreamPrefetchProvider : AdiXtream() {
    companion object {
    ...
    private val playbackDelegate = AdiXtreamPlaybackFixedProvider()
    ...
    private fun forwarder(
        ...
        if (!duplicateIdlix) {
            ...
            callback(link)
            Log.i("AdiXtream", "[IDLIX-PREFETCH] callback|FAMILY=$key|LINKS=$count|FAMILIES=${families.size}")
    ...
    private suspend fun loadFastIdlix(
        ...
        val orgTitle = payload.optString("originalTitle").takeIf { it.isNotBlank() }
        val year = if (payload.has("year") && !payload.isNull("year")) payload.optInt("year") else null
        val isSeries = payload.optBoolean("isTvSeries", false)
    ...
    override suspend fun loadLinks(
                ...
                supervisorScope {
                    ...
                    val prefetchedIdlix = launch {
                        try {
                            ...
                            Log.e("AdiXtream", "[IDLIX-PREFETCH] ${error.javaClass.simpleName}: ${error.message}")
                        }
                    }
        ...
        Log.i("AdiXtream", "[IDLIX-PREFETCH] release|FAMILIES=${families.joinToString(",")}|LINKS=${emitted.get()}")
        emitted.get() > 0
    }
