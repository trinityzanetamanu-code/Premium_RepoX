...
class AdiDrakorPlaybackFixedProvider : AdiDrakor() {
    ...
    private fun graceForwarder(
        ...
        if (output != null) {
            ...
            callback(output)
            if (position == 1) {
                Log.i("AdiDrakor", "[FAST-GRACE] first=${output.source}|ACTION=start-grace")
        ...
        supervisorScope {
            val baseJob = launch {
                try {
                    ...
                    if (error is CancellationException) throw error
                    Log.e("AdiDrakor", "[FAST-GRACE] base sources error: ${error.javaClass.simpleName}: ${error.message}")
                }
            }
    ...
    override suspend fun loadLinks(
        ...
        val loaderJob = launch {
            try {
                ...
                if (error is CancellationException) throw error
                Log.e("AdiDrakor", "[FAST-GRACE] resolver error: ${error.javaClass.simpleName}: ${error.message}")
            } finally {
