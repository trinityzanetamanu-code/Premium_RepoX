...
class AdiFilmSemiPlaybackFixedProvider : AdiFilmSemi() {
    ...
    private fun firstReadyForwarder(
        ...
        if (output != null) {
            ...
            callback(output)
            if (position == 1) {
                Log.i("AdiFilmSemi", "[FIRST-READY] source=${output.source}|ACTION=release-player")
    ...
    private suspend fun loadVidSrcSource(
        ...
        try {
            ...
            Log.e("AdiFilmSemi", "[VIDSRC] gagal resolve: ${error.javaClass.simpleName}: ${error.message}")
        }
    }
        ...
        supervisorScope {
            val baseJob = launch {
                try {
                    ...
                    if (error is CancellationException) throw error
                    Log.e("AdiFilmSemi", "[FIRST-READY] base sources error: ${error.javaClass.simpleName}: ${error.message}")
                }
            }
    ...
    override suspend fun loadLinks(
        ...
        playbackScope.launch {
            try {
                ...
                if (error is CancellationException) throw error
                Log.e("AdiFilmSemi", "[FIRST-READY] background resolver error: ${error.javaClass.simpleName}: ${error.message}")
            } finally {
                if (!firstReady.isCompleted) firstReady.complete(emitted.get() > 0)
                Log.i("AdiFilmSemi", "[FIRST-READY] background complete|TOTAL_LINKS=${emitted.get()}")
            }
        }
