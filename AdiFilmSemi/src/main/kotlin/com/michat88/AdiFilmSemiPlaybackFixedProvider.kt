...
class AdiFilmSemiPlaybackFixedProvider : AdiFilmSemi() {
    ...
    private fun graceForwarder(
        ...
        if (output != null) {
            ...
            if (position == 1) {
                Log.i("AdiFilmSemi", "[THREE-SOURCE] first=${output.source}|ACTION=bounded-wait")
        ...
        supervisorScope {
            val baseJob = launch {
                try {
                    ...
                    if (error is CancellationException) throw error
                    Log.e("AdiFilmSemi", "[THREE-SOURCE] base sources error: ${error.javaClass.simpleName}: ${error.message}")
                }
            }
    ...
    override suspend fun loadLinks(
        ...
        val loaderJob = launch {
            try {
                ...
                if (error is CancellationException) throw error
                Log.e("AdiFilmSemi", "[THREE-SOURCE] resolver error: ${error.javaClass.simpleName}: ${error.message}")
            } finally {
        ...
        if (loaderJob.isActive && sourceKeys.size >= TARGET_SOURCE_COUNT) {
            Log.i("AdiFilmSemi", "[THREE-SOURCE] all-families-ready|FAMILIES=${sourceKeys.size}")
